...
def box(text, title=None, alignment='center', width=80):
    ...
    text = text.replace('\t', '    ') # set uniform tab width for advance knowledge
...
def split_last_full_word(line_text):
    if line_text[-1] != '\n': # negation first b/c far more common
        try: # quicker and cheaper than conditional since lines w/o ' ' are so rare
            final_space_index = line_text.rindex(' ')
            return line_text[:final_space_index], line_text[final_space_index + 1:]
        except ValueError:
            return line_text, ''
    else:
